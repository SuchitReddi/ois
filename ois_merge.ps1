...
Write-Host "`nBy " -NoNewLine
Write-Host "Suchit" -ForegroundColor DarkRed
}
...
Function Show-Animated-Text {
    Param (
        [string]$text,
        [int]$delay = 0.65,  # Default delay of 0 milliseconds for fastest animation
	...
	function Get-VTPolling {
		...
		while (-not $scanReady -and $try -lt $maxTries) {
			...
			try {
				...
				if ($lastanalysistimeepoch -and $lastanalysistimeepoch -ne 0 -and $timeago -ne "55 years ago") {
					$scanReady = $true
				}
			} catch {
...
Function Lookup-Handler {
    ...
    foreach ($ioc in $iocs) {
        ...
        if ($type -eq "domain") {
            ...
            $url = $ioc
            if ($useurlscan -eq "Y") {
                $usresult = Invoke-Urlscan -url $url -configPath $configPath
...
if (-not $configPath) {
    ...
    Write-Host "`nconfig.json was not found. A default config.json has been created in: $configPath" -ForegroundColor Red
...
Do {
    ...
    $iocInput = Read-Host "`nEnter maximum $iocLimit IOCs (i-info, e-edit, c-clear, q-quit)"
    ...
    if ($iocInput -match '^(?i)c$') {
        Clear-Host
        Show-Logo
        Write-Host "`n`nGiving you a clean slate... make it colorful again!" -ForegroundColor Magenta
        Write-Host ""
        Write-Host ""
        Start-Sleep -Seconds 2
        Clear-Host
    ...
    if ($iocInput -match '^(?i)i$') {
        ...
        Show-Animated-Text -text "2) Maximum of 4 IOCs are recommended to limit excessive resource consumption if you opt for opening results in browser. If more than four are given, a confirmation to proceed will be displayed."
        ...
        Show-Animated-Text -text "6) The executable can be run from anywhere, but the config file created needs to be in the same directory."
        Show-Animated-Text -text "7) If you submit Virus Total API Key, you can get many details directly in console. IOCs will be submitted to VT for reanalyzing when the script is run."
        Show-Animated-Text -text "8) If you have URLScan account, you can use the API to get a live screenshot for the URL. You will be prompted to choose if you want to use it or not in the beginning. If you want to change the choice later, you can edit it from e."

        Write-Host "`nKnown Issues: " -ForegroundColor Red
        Show-Animated-Text -text "--> IOCs can't be seperated by new line character as powershell works differently in those cases."
    ...
    If (-not $iocInput) {
        Write-Host "Enter valid IOCs. With correct IOCs come great results!" -ForegroundColor Red
        Continue
    }
    ...
    If (-not $iocs) {
        Write-Host "No valid IOCs found. With correct IOCs come great results!" -ForegroundColor Red
        Continue
    }
